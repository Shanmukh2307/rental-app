    ...
    try{
       const{
        ...
        if(latitude && longitude){
            ...
            const degrees = radiusInKilometers / 111.32; // Approximate conversion from km to degrees
            whereConditions.push(
    ...
    try{
      ...
      const geocodingUrl = `https://nominatim.openstreetmap.org/search?${new URLSearchParams(
        ...
        const geocodingResponse = await axios.get(geocodingUrl,{
            headers: {
                "User-Agent": "RentalApp (shanmukhv78@gmail.com)"
            },
        });
            ...
            data: {
                ...
                amenities:
                   typeof propertyData.amenities === "string"
                      ? (propertyData.amenities as string).split(',') : [],  
                highlights:
                   typeof propertyData.highlights === "string"
                      ? (propertyData.highlights as string).split(',') : [],  
                isPetsAllowed:
                     propertyData.isPetsAllowed === "true" ? true : false,
                isParkingIncluded:
                     propertyData.isParkingIncluded === "true" ? true : false,
